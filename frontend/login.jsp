    ...
    <div class="auth-wrapper glass-panel">
        <div class="auth-image-side">
            <div class="image-overlay">
                <h2>Welcome to ToyVerse</h2>
                <p>Your premium destination for the finest toys and games.</p>
            </div>
        </div>
        <div class="auth-form-side">
            <div class="auth-header">
                ...
                <h1>Welcome Back</h1>
                <p>Sign in to access your cart and track orders.</p>
            </div>
...
<style>
    ...
    .emoji-icon { font-size: 64px; display: inline-block; margin-bottom: 20px; animation: bounce 3s ease-in-out infinite; }
    @keyframes bounce { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-10px); } }
    .auth-header h1 { font-size: 32px; font-weight: 900; color: #1E293B; letter-spacing: -1px; margin-bottom: 12px; }
    .auth-header p { color: #64748B; font-size: 16px; font-weight: 500; }

    .form-group { margin-bottom: 24px; text-align: left; }
    .form-group label { display: block; font-size: 14px; font-weight: 700; color: #475569; margin-bottom: 10px; }
    .form-group input { width: 100%; padding: 18px 24px; background: #F8FAFC; border: 2px solid transparent; border-radius: 16px; font-size: 15px; color: #1E293B; outline: none; transition: all 0.3s ease; font-weight: 500; }
    .form-group input:focus { border-color: var(--primary); background: white; box-shadow: 0 0 0 4px rgba(79, 70, 229, 0.1); }
    
    .password-toggle { position: absolute; right: 20px; top: 18px; cursor: pointer; color: #94A3B8; transition: color 0.3s; }
    .password-toggle:hover { color: var(--primary); }

    .btn-auth { width: 100%; padding: 20px; border: none; border-radius: 50px; background: var(--primary); color: white; font-size: 16px; font-weight: 800; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 10px 25px rgba(79, 70, 229, 0.3); margin-top: 12px; }
    ...
    .auth-links { margin-top: 40px; text-align: center; padding-top: 24px; border-top: 1px solid rgba(0,0,0,0.05); }
...
<script>
    ...
    async function login() {
        ...
        if (!username || !password) { if(typeof showToast === 'function') showToast('Please fill in all fields', true); return; }

        btn.classList.add('loading'); btn.textContent = 'Signing in...';

        try {
            const params = new URLSearchParams({ username, password });
            const response = await fetch('/api/login', { method: 'POST', body: params });
            const data = await response.json();

            if (data.success) {
                if(typeof showToast === 'function') showToast('Login successful! Redirecting...');
                setTimeout(() => { window.location.href = data.role === 'ADMIN' ? '/admin/dashboard' : '/toys'; }, 1000);
            } else {
